...
| 19 | Inbox me reply type karo, Send dabao — **kuch nahi hota**: na bubble, na error, na DB row ("ajeeb tamasha") | WhatsApp conversation ka **24h window band** tha → controller `back()->with('error')` return karta tha → axios redirect chupchaap follow karta hai → Inertia HTML (200) milta hai → `res.data.message` undefined → UI silent no-op. Row bhi nahi banti kyunki return `Message::create` se pehle hota tha | Fixed — JSON requests ab `422 + {error}` return karte hain; UI ka red banner error dikhata hai. Non-WhatsApp channels aur open-window conversations pe asli send chalta hai. Dev note: `back()` JSON caller ke liye kabhi use na karo |
| 20 | Worker log me `Permission denied: storage/logs/errors-*.log` | Logs root ne banaye, worker `www` user hai | `chown -R www:www storage/logs` (pull/restart ke baad bhi ek dafa) |
| 21 | Webhooks aati hain (`webhook.dispatching` logs), messages kabhi Inbox me nahi aate — **koi error bhi nahi** | Worker mara hua tha: `queue:restart` exit code 0 pe worker band karta hai aur supervisord ka default `autorestart=unexpected` code-0 exit ko restart nahi karta; `--max-time=3600` har ghante khud exit karwata hai | Debug funnel: 1) `Queue::size('instagram')` pending count 2) `queue:failed` 3) `ps aux \| grep queue:work` 4) `grep webhook.dispatching laravel.log`. Fix: daemon conf me `autorestart=true` + command se `--max-time` hatao + panel se worker Start |
